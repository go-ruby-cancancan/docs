...
`go-ruby-cancancan/cancancan` reimplements Ruby's `cancancan` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-cancancan/cancancan` directly, with no Ruby runtime;
...
Because `cancancan` is pure Go with cgo disabled, it also compiles to **WebAssembly**
